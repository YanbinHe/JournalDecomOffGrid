...
while ~converged
    ...
    track_gamma = [track_gamma,alpha];
    track_middle = [track_middle,(paras.middle + beta)];
...
%     [pksv,idx] = findpeaks(alpha); % need to be refined
%     [~,pksvposi] = maxk(pksv,updatecomp);
%     idx = idx(pksvposi);
...
res.beta = beta;
res.gamma = alpha;
res.track_gamma = track_gamma;
res.track_middle = track_middle;
