function [gamma_est,u_grid_updated,Agrid_updated]=gridPtAdjPks(gamma_est,K,u_grid_updated,Agrid_updated,spos,M,Ryoyo,lambda,prob)
...
% iSigma=eye(M)/Sigma;
for iterGdPtAdPks=1:7 % grid point adjustment around peaks iteration
...
%         iSigma_mi=iSigma-wi_vec_remove*wi_vec_remove'/(-1/gamma_est(m_iterK)+Agrid_updated(:,m_iterK)'*wi_vec_remove);
        ...
        Aadpt_grid=generate_colFuncs(prob,u_candidates'); %exp(-1j*pi*spos'*);
        ...
        if valneigh<0
            ...
            Agrid_updated(:,m_iterK)=generate_colFuncs(prob,u_candidates(indneigh)');%exp(-1j*pi*spos'*u_candidates(indneigh));
